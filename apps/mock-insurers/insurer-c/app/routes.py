...
@router.get("/health")
def health_check():
    return {"status": "ok", "insurer": "TATA AIG", "gateway": "insurer_c"}
...
def submit_quote(
    ...
    context = {
        ...
        "engine_capacity_cc": int(engine_capacity_cc or 1200),
    }
...
def create_admin_product(
    ...
    base_rate_percent: float = Form(3.5),
    min_premium: float = Form(18900.0),
    db: Session = Depends(get_db)
):
    prod = Product(
        name=name,
    ...
    notify_synova_platform(
        ...
        message=f"TATA AIG Assurance has launched '{name}' (Base Rate: {base_rate_percent}%, Starting from ₹{int(min_premium):,}) with NCB Protector benefits.",
        ...
        try:
            with open(SHARED_NOTIF_FILE, "r", encoding="utf-8") as f:
                ...
                if isinstance(shared, list):
                    for n in shared:
                        if n.get("insurer_name") == "TATA AIG Assurance":
                            results.append(n)
    ...
    for p in products:
        if p.name not in seen_products:
            results.append({
                ...
                "message": f"TATA AIG Assurance has launched '{p.name}' (Category: {p.insurance_type.upper()}, Base Rate: {p.base_rate_percent}%) with NCB Protector benefits.",
                "insurer_name": "TATA AIG Assurance",
                "product_name": p.name,
...
def delete_admin_product(product_id: int, request: Request, db: Session = Depends(get_db)):
    ...
    if prod:
        ...
        notify_synova_platform(
            ...
            message=f"TATA AIG Assurance has retired policy '{prod_name}' from the live catalog.",
            insurer_name="TATA AIG Assurance",
