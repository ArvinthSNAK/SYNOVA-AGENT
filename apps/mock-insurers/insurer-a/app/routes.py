...
_jinja_env = Environment(
    ...
    autoescape=True,
    cache_size=0,  # avoid a Jinja2/Python 3.14 template-cache compatibility bug
)
...
def create_admin_product(
    ...
    base_rate_percent: float = Form(3.0),
    min_premium: float = Form(16500.0),
    db: Session = Depends(get_db)
):
    prod = Product(
        name=name,
    ...
    notify_synova_platform(
        ...
        message=f"ICICI Lombard General has launched '{name}' (Base Rate: {base_rate_percent}%, Starting from ₹{int(min_premium):,}) with Zero Depreciation benefits.",
        ...
        try:
            with open(SHARED_NOTIF_FILE, "r", encoding="utf-8") as f:
                ...
                if isinstance(shared, list):
                    for n in shared:
                        if n.get("insurer_name") == "ICICI Lombard General":
                            results.append(n)
    ...
    for p in products:
        if p.name not in seen_products:
            results.append({
                ...
                "message": f"ICICI Lombard General has launched '{p.name}' (Category: {p.insurance_type.upper()}, Base Rate: {p.base_rate_percent}%) with Zero Depreciation benefits.",
                "insurer_name": "ICICI Lombard General",
                "product_name": p.name,
...
def delete_admin_product(product_id: int, request: Request, db: Session = Depends(get_db)):
    ...
    if prod:
        ...
        notify_synova_platform(
            ...
            message=f"ICICI Lombard General has retired policy '{prod_name}' from the live catalog.",
            insurer_name="ICICI Lombard General",
