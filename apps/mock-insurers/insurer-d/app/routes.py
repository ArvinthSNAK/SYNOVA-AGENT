...
def submit_quote(
    ...
    engine_capacity_cc: int = Form(1497),
    has_anti_theft: str = Form("0"),
    ...
    # Normalize has_anti_theft
    if str(has_anti_theft).lower() in ("true", "1", "yes"):
        anti_theft_int = 1
    else:
        anti_theft_int = 0
    ...
    context = {
        ...
        "engine_capacity_cc": int(engine_capacity_cc or 1497),
        "has_anti_theft": anti_theft_int,
    }
...
def create_admin_product(
    ...
    base_rate_percent: float = Form(2.8),
    min_premium: float = Form(15800.0),
    db: Session = Depends(get_db)
):
    prod = Product(
        name=name,
    ...
    notify_synova_platform(
        ...
        message=f"HDFC ERGO General has launched '{name}' (Base Rate: {base_rate_percent}%, Starting from ₹{int(min_premium):,}) with Emergency Fuel & Zero Dep benefits.",
        ...
        try:
            with open(SHARED_NOTIF_FILE, "r", encoding="utf-8") as f:
                ...
                if isinstance(shared, list):
                    for n in shared:
                        if n.get("insurer_name") == "HDFC ERGO General":
                            results.append(n)
    ...
    for p in products:
        if p.name not in seen_products:
            results.append({
                ...
                "message": f"HDFC ERGO General has launched '{p.name}' (Category: {p.insurance_type.upper()}, Base Rate: {p.base_rate_percent}%) with Emergency Fuel & Zero Dep benefits.",
                "insurer_name": "HDFC ERGO General",
                "product_name": p.name,
...
def delete_admin_product(product_id: int, request: Request, db: Session = Depends(get_db)):
    ...
    if prod:
        ...
        notify_synova_platform(
            ...
            message=f"HDFC ERGO General has retired policy '{prod_name}' from the live catalog.",
            insurer_name="HDFC ERGO General",
